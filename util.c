...
static int usbGetStringAscii(usb_dev_handle *dev, int index, int langid, char *buf, int buflen)
{
	...
	rval = usb_control_msg(dev, USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR, (USB_DT_STRING << 8) + index, langid, buffer, sizeof(buffer), 1000);
	if (rval < 0)
	{
		return rval;
	}
...
static void xum1541_cleanup(usb_dev_handle **usbHandle, char *msg, ...)
{
	va_list args;
...
static int xum1541_validate_device(struct usb_device *dev, void *arg)
{
	...
	if (dev->descriptor.idVendor != XUM1541_VID || dev->descriptor.idProduct != XUM1541_PID)
	{
	...
	len = usbGetStringAscii(handle, dev->descriptor.iProduct, 0x0409, string, sizeof(string) - 1);
	if (len < 0)
	{
	...
	{
		xum1541_cleanup(&handle, "error: xum1541 device \"%s\" doesn't match product string\n", xumProduct);
		return USBDEV_IGNORE;
	}

	verbose_print("    xum1541 name: %s\n", string);

	len = usbGetStringAscii(handle, dev->descriptor.iSerialNumber, 0x0409, string, sizeof(string) - 1);
	if (len < 0 && PortNumber != 0)
	{
		// we need the serial number, when PortNumber is not 0
		xum1541_cleanup(&handle, "error: cannot query serial number: %s\n", usb_strerror());
		return USBDEV_IGNORE;
	}
...
// USB bus enumeration
static int xum1541_enumerate(usb_dev_handle **usbHandle, DeviceValidateFn_t validateFn, void *arg)
{
	struct usb_bus *bus;
...
usb_dev_handle * xum1541_find_device(int PortNumber, char *devNameBuf, int nameBufSize)
{
