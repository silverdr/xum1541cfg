...
static int GetDFUDevice(char *deviceType, dfu_device_t *devHandle, struct programmer_arguments *args);
static int CheckFirmwareVersion(usb_dev_handle *usbHandle, int fileModel, int fileVersion);
static int EraseFlash(dfu_device_t *dev, struct programmer_arguments *args);
static int UpdateFlash(dfu_device_t *dev, struct programmer_arguments *args, char *firmwareFile);
...
static int16_t *ihex_search(int16_t *buf, int bufSize, uint8_t *pattern, uint8_t patternSize);


// xum1541 util.c
int xum1541_get_model_version(usb_dev_handle *handle, int *model, int *version);
usb_dev_handle *xum1541_find_device(int PortNumber, char *devNameBuf, int devNameBufSize);
...
static int RunUpdate(dfu_device_t *devHandle, char *firmwareFile, char *deviceType, int forceFlag)
{
	...
	if (ret == 0)
	{
		...
		if (ret != 0)
		{
			...
			if (!forceFlag)
			{
				fprintf(stderr, "add the -f flag if you really have the right firmware.\n");
				return -1;
			}
	...
	{
		fprintf(stderr, "warning: no xum1541 found, continuing to look for devices in DFU mode\n");
	}
	...
	if (ret != 0)
	{
		fprintf(stderr, "warning: can't restart xum1541 but update was ok. Try re-plugging it.\n");
		return ret;
	}
...
static int SetSerial(int newSerial)
{
	// TODO
...
static int GetXumDevice(char *deviceType, usb_dev_handle **usbHandle)
{
	...
	// Find xum1541 device or Atmel DFU device.
	*usbHandle = xum1541_find_device(0/*serial*/, devNameBuf, sizeof(devNameBuf));
	if (*usbHandle == NULL)
	{
		return -1;
	}
...
static int SetDFUMode(usb_dev_handle *usbHandle)
{
	...
	nBytes = usb_control_msg(usbHandle, USB_TYPE_CLASS | USB_ENDPOINT_OUT, XUM1541_ENTER_BOOTLOADER, 0, 0, NULL, 0, 1000);
	if (nBytes < 0)
	{
...
static int GetDFUDevice(char *deviceType, dfu_device_t *devHandle, struct programmer_arguments *args)
{
	...
	for (retries = 30; retries != 0; retries--) {
		retPtr = dfu_device_init(args->vendor_id, args->chip_id, devHandle, args->initial_abort, args->honor_interfaceclass);
		if (retPtr != NULL)
		{
			break;
		}
...
static int CheckFirmwareVersion(usb_dev_handle *usbHandle, int fileModel, int fileVersion)
{
	...
	{
		fprintf(stderr, "error: device type %d does not match firmware type %d\n", devModel, fileModel);
		return -1;
	}
	...
	{
		fprintf(stderr, "note: device has version %d but firmware is not newer (version %d)\n", devVersion, fileVersion);
		return 1;
	}
